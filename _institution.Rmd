...
try({
	if(length(chief_admins)/length(unique(chief_admins))<4 && length(chief_admins)>2 && length(unique(chief_admins))>2) {
		admin_vector <- c()
...
* **Student to tenure-stream faculty ratio**: `r most_current_info['Full time undergrad enrollment divided by tenure stream faculty']` (undergrads to tenure-stream faculty)
* **Student to faculty ratio**: `r most_current_info['Full time undergrad enrollment divided by total instructors']` (undergrads to all faculty)
...
`r ifelse(is.finite(as.numeric(most_current_info['Full time undergrad enrollment divided by dorm capacity'])), ifelse(as.numeric(most_current_info['Full time undergrad enrollment divided by dorm capacity'])>0, paste0("* **Dorm capacity**: There are enough dorm beds for ", round(100*as.numeric(most_current_info['Full time undergrad enrollment divided by dorm capacity']),1), "% of the full time undergrads"), ""), "")`
* **Meal plan**: `r most_current_info['Institution provides board or meal plan']`
* **Covid vaccination requirement for students**: `r ifelse(most_current_info['AllStudentsVaccinatedAgainstCovid19']=="Yes", "At some point during the pandemic (this may have changed), this institution required students to be vaccinated against covid", "This institution was never reported as requiring covid vaccination for students")`
...
* **Abortion in this state**: `r most_current_info['Abortion']`
...
* **State rep support for contraception**: `r round(100*as.numeric(most_current_info['Proportion of reps voting in favor of respect for right to contraception act']),1)`% of US reps from this state voted in favor of legal protections for contraception.
* **State rep support for recognizing same-sex and interracial marriage**: `r round(100*as.numeric(most_current_info['Proportion of reps voting in favor of respect for marriage act']),1)`% of US reps from this state voted in favor of requiring states to recognize same-sex and interracial marriages performed in other states.
...
* **Urbanization**: `r most_current_info['Degree of urbanization Urban centric locale']`
...
Graduation rates for bachelor's degrees within 150% of normal time (6 years for a 4-year degree). Note that this uses US federal demographic data: it only has two genders and a specified set of ethnicities and races.
...
Tenure track faculty are those who are eligible for tenure. This includes both tenure stream and non-tenure stream faculty. Once faculty get tenure, they are (generally) protected from being fired for intellectual reasons, helping to ensure their freedom in teaching and research. They can still lose their positions for misconduct, financial problems, or other reasons. Note that this chart uses US federal demographic data: it only has two genders and a specified set of ethnicities and races.
...
```{r, eval=TRUE}
	try({
	degree_types <- c("Certificate", "Associates", "Bachelors", "Masters", "Doctorate")
	
	#majors %>% arrange(Degree) %>% dplyr::select(-Degree) %>% knitr::kable(align="c") %>% #kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% #kableExtra::pack_rows(index = table(majors$Degree))

	#for (degree_type in degree_types) {
	#	focal_degree <- subset(majors, Degree=degree_type)
	#	if(nrow(focal_degree)>0) {
		#	print("\n## ", degree_type, "\n\n")
		#	focal_degree %>% knitr::kable(align="c") %>% kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
	#	}	
	#}
	...
	DT::datatable(as.matrix(majors)) %>% DT::formatStyle(columns = names(data))
	}, silent=TRUE)
