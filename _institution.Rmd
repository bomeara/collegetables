...
* This college is located in a `r params$overview_table['Locale']`
...
* For abortion access, as of July, 2022, the state this institution is in was ranked as having "`r gsub("\\d\\: ", "", tolower(params$overview_table['Abortion restrictions'][1,1]))`" by the [Guttmacher Institute](https://states.guttmacher.org/policies/), though this may change.
* [California `r ifelse(params$overview_table["Anti-LGBTQ+ state laws" ]=="Yes", "considers", "does not consider")` this state to have anti-LGBTQ+ laws](https://oag.ca.gov/ab1887). `r ifelse(params$overview_table["Anti-LGBTQ+ state laws" ]=="Yes", "This means that California will not approve or require state-sponsored travel to this state.", "")`
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
df_to_plot <- focal_nearby[,c('Institution Name', "Student-to-faculty ratio", 'StudentToTenureTrackRatio')]
colnames(df_to_plot) <- c("Institution", "StudentsToAllFaculty", "StudentsToTTFaculty")
df_to_plot[,2] <- as.numeric(df_to_plot[,2])
df_to_plot[,3] <- as.numeric(df_to_plot[,3])
print(
	ggplot(df_to_plot, aes(x=StudentsToAllFaculty, y=StudentsToTTFaculty, label=Institution)) +
	geom_point() + geom_text_repel() + xlab("Students per instructor (lower is better)") + ylab("Students per tenure track instructor (lower is better)")
	)
```
