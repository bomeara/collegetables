...
`r ifelse(focal_raw['AAUP_Censure']=="Yes", '<h3 style="color:Red;">Warning</h3><p> **"Unsatisfactory conditions of academic freedom and tenure have been found to prevail at this institution"** according to the <a href="https://www.aaup.org/our-programs/academic-freedom/censure-list">AAUP</a> (as of July 31, 2022)</p>', "")`

`r ifelse(focal_raw["Anti-LGBTQ+ state laws" ]=="Yes", '<h3 style="color:Red;">Warning</h2><p />**California considers the state this institution is in to have one or more anti-LGBTQ+ laws** (as of July 31, 2022). It prohibits California-sponsored travel to this state as a safety measure. See more <a href="https://oag.ca.gov/ab1887">here</a>.</p>', "")`


`r ifelse(focal_raw["Undergrads per tenure-track professor" ]>1000, '<h3 style="color:Red;">Warning</h2><p />**There are extremely few tenure-track faculty relative to the number of students.** For something like an art or music school where instruction is done nearly entirely by rotating artists or musicians, this may be fine; for other schools, it can indicate a <a href="https://www.aaup.org/issues/tenure">risk to academic freedom</a> and thus educational quality, as faculty members *may* be able to lose their positions because of their speech, publications, or research findings.', "")`
...
for (field_index in seq_along(fields_to_compare)) {
	...
	for (list_index in sequence(length(comparison_list))) {
		...
		if(render_as_violins) { # violin plots
			...
			#if(grepl("Undergrads per", focal_field) | grepl("Admission", focal_field) ) { # we could reverse for admission. But rejecting many students who want to go there, and who would be successful there, is not great from a student perspective
			if(grepl("Undergrads per", focal_field) | grepl("Reported|discipline|arrest", focal_field)) {
...
comparison_df['Endowment assets per FTE', 1] <- paste0('$', comparison_df['Endowment assets per FTE', 1])
comparison_df['Average net price-students awarded grant or scholarship aid', 1] <- paste0('$', comparison_df['Average net price-students awarded grant or scholarship aid', 1])
...
comparison_df['Fraction of tenure track faculty who are women',1] <- paste0(100*round(as.numeric(comparison_df['Fraction of tenure track faculty who are women',1])), '%')
rownames(comparison_df) <- gsub('Fraction of tenure track faculty who are women', 'Tenure-track faculty who are women',  rownames(comparison_df))

comparison_df['Fraction of non-tenure track faculty who are women',1] <- paste0(100*round(as.numeric(comparison_df['Fraction of non-tenure track faculty who are women',1])), '%')
rownames(comparison_df) <- gsub('Fraction of non-tenure track faculty who are women', 'Non-tenure-track faculty who are women',  rownames(comparison_df))


comparison_df['Fraction of tenure track faculty who are BIPOC',1] <- paste0(100*round(as.numeric(comparison_df['Fraction of tenure track faculty who are BIPOC',1])), '%')
rownames(comparison_df) <- gsub('Fraction of tenure track faculty who are BIPOC', 'Tenure-track faculty who are Black, Indigenous, and/or people of color',  rownames(comparison_df))

comparison_df['Fraction of non-tenure track faculty who are BIPOC',1] <- paste0(100*round(as.numeric(comparison_df['Fraction of non-tenure track faculty who are BIPOC',1])), '%')
rownames(comparison_df) <- gsub('Fraction of non-tenure track faculty who are BIPOC', 'Non-tenure-track faculty who are Black, Indigenous, and/or people of color',  rownames(comparison_df))
...
# pct_data <- dplyr::select(Students_by_state, c("fips", "Percent_of_18_yo_from_this_state_enrolled_here"))
