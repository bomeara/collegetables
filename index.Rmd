...
DT::datatable(overview, rownames=FALSE, filter="top", escape=FALSE, extensions = c('Buttons', 'FixedColumns', 'FixedHeader'), options = list(
    ...
    buttons = list(
		...
		list(extend= "colvisGroup", text="Reset", show=-1+sequence(ncol(overview))),
		list(
			extend= "colvisGroup",
			text="Admissions",
			show=c(0, -1+match(c("Admission", "Yield"),colnames(overview))), 
			hide=c(-1+match(c("Name", "Admission", "Yield"),colnames(overview), incomparables=TRUE))
		), # need to remember javascript starts with 0
		list(
			extend= "colvisGroup",
