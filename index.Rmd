...
if(nrow(all_colleges)>0) {
	...
	Overview <- c("Name", "Institution type", "Institution sector" , "Avg net price for students with grants/scholarships", "Average federal loan", "Average non-federal loan", "Undergraduate enrollment",  "Graduate enrollment", "Athletic conference" )

	Admissions_and_outcomes <- c("Admission", "Yield", "First year retention", "Transfer", "Graduation")
...
AllFields <- c(Overview, Admissions_and_outcomes, Location, Safety, AcademicResources, Misconduct, Finances, Degrees)
	...
	all_colleges_cleaned$State <- state.name[match(all_colleges_cleaned$State,state.abb)]
