...
finances$Net <- finances$Total.revenues.and.investment.return - finances$Total.expenses
finances_plotting <- finances %>% dplyr::select('IPEDS.Year', 'Net', 'Total.net.assets')
print(ggplot(finances_plotting, aes(x=IPEDS.Year, y=Net)) + geom_line() + ylab("Revenue minus expenses ($)") + xlab("Year"))
print(ggplot(finances_plotting, aes(x=IPEDS.Year, y=Net)) + geom_link2(aes(colour = after_stat(ifelse(y > 0, "positive", "negative"))))) + scale_colour_manual(values=c("red", "black"))
...
print(ggplot(revenue_longer, aes(x=IPEDS.Year, y=value, fill=name)) + geom_area())
```
