...
for (i in sequence(nrow(Students_by_state))) {
	Students_by_state$Undergrad_Count[i] <- unname(unlist(params$raw_table[paste0("Count of first-time undergrads from ", Students_by_state$Name[i])]))
	Students_by_state$Percent_of_18_yo_from_this_state_enrolled_here[i] <- unname(unlist(params$raw_table[paste0("Percentage of all 18 year olds in the focal state enrolling in this college from ", Students_by_state$Name[i])]))
