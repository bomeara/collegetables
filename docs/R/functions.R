AggregateIPEDS <- function() {
	...
	walkability_aggregated <- walkability %>% group_by(CBSA_Name) %>% summarise(NatWalkInd=round(median(NatWalkInd),1), Population=sum(TotPop), HousingUnits = sum(CountHU, na.rm=TRUE), DistanceToTransit = round(median(D4A, na.rm=TRUE)))
...
GetOverviewColumns <- function(college_data) {
	overview <- as.data.frame(college_data %>% select("Institution Name", "Sector of institution", "CollegeType","City location of institution","State abbreviation",  "Percent admitted - total", "Admissions yield - total", "Full-time retention rate  2019",  "Graduation rate  total cohort", "Undergraduate enrollment",  "StudentToTenureTrackRatio", "Degree of urbanization (Urban-centric locale)", "NatWalkInd", "DistanceToTransit", "BannedCATravel", "Abortion restrictions", "NumberOfPhysicalBooksPerUndergrad", "AllStudentsVaccinatedAgainstCovid19", "AllEmployeesVaccinatedAgainstCovid19", "InMisconductDatabase", "Percent of undergraduate enrollment that are American Indian or Alaska Native", "Percent of undergraduate enrollment that are American Indian or Alaska Native", "Percent of undergraduate enrollment that are American Indian or Alaska Native", "Percent of undergraduate enrollment that are American Indian or Alaska Native", "Percent of undergraduate enrollment that are Black or African American" , "Percent of undergraduate enrollment that are Hispanic/Latino" ,  "Percent of undergraduate enrollment that are White" ,  "Percent of undergraduate enrollment that are two or more races"  , "Percent of undergraduate enrollment that are Race/ethnicity unknown"  , "Percent of undergraduate enrollment that are Nonresident Alien" ,  "Percent of undergraduate enrollment that are women",   "Percent of undergraduate enrollment that are women", "Grand total men (Full-time instructional with faculty status)", "Grand total women (Full-time instructional with faculty status)" , "American Indian or Alaska Native men (Full-time instructional with faculty status)" ,    "American Indian or Alaska Native women (Full-time instructional with faculty status)" , "Asian men (Full-time instructional with faculty status)"   ,"Asian women (Full-time instructional with faculty status)" ,"Black or African American men (Full-time instructional with faculty status)"  , "Black or African American women (Full-time instructional with faculty status)", "Hispanic or Latino men (Full-time instructional with faculty status)" ,"Hispanic or Latino women (Full-time instructional with faculty status)"  ,"Native Hawaiian or Other Pacific Islander men (Full-time instructional with faculty status)" ,"Native Hawaiian or Other Pacific Islander women (Full-time instructional with faculty status)"  , "White men (Full-time instructional with faculty status)" ,"White women (Full-time instructional with faculty status)"    ,   "Two or more races men (Full-time instructional with faculty status)"  ,"Two or more races women (Full-time instructional with faculty status)" ,"Race/ethnicity unknown men (Full-time instructional with faculty status)"      ,"Race/ethnicity unknown women (Full-time instructional with faculty status)"   ,"Nonresident alien men (Full-time instructional with faculty status)", "Nonresident alien women (Full-time instructional with faculty status)"  ))
